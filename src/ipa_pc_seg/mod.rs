...
impl<G: AffineCurve, D: Digest> InnerProductArgPC<G, D> {
    ...
    fn cm_commit(
        ...
        let scalars_bigint = scalars.par_iter()
            .map(|s| s.into_repr())
            .collect::<Vec<_>>();
        let mut comm = VariableBaseMSM::multi_scalar_mul(&comm_key[0..core::cmp::min(scalars.len(), comm_key.len())], &scalars_bigint);
    ...
    ) -> Result<(SuccinctCheckPolynomial<G::ScalarField>, G), Error>
    {
        ...
        let batch_comm = batch_proof.batch_comm.clone();
        ...
        let labeled_batch_commitment = LabeledCommitment::new(
            format!("Batch"),
            Commitment { comm: batch_comm.clone(), shifted_comm: None },
            None
        );
...
impl<G: AffineCurve, D: Digest> PolynomialCommitment<G::ScalarField> for InnerProductArgPC<G, D> {
    ...
    > {
        ...
        for labeled_polynomial in polynomials {
            ...
            let n = ck.comm_key.len();
            let p = polynomial.coeffs.len();

            // split poly in segments and commit all of them without shifting
            let comm: Vec<G> = (0..p/n + if p % n != 0 { 1 } else { 0 }).into_par_iter().map(
                |i| {
                    Self::cm_commit(
                        &ck.comm_key,
                        &polynomial.coeffs[i * n..core::cmp::min((i + 1) * n, p)],
                        Some(ck.s),
        ...
        Self::Randomness: 'a,
    {
            ...
            polys_iter.zip(comms_iter.zip(rands_iter))
        {
            ...
            for i in 0..p/n + if p % n != 0 { 1 } else { 0 } {
                let is = i * n;
                let poly_single = Polynomial::from_coefficients_slice(&polynomial.coeffs[i * n..core::cmp::min((i + 1) * n, p)]);
        ...
        Self::Commitment: 'a,
    {
        ...
        let n = ck.comm_key.len();
        let p = batch_polynomial.coeffs.len();

        // Commitment of the h(X) polynomial
        let batch_comm: Vec<G> = (0..p/n + if p % n != 0 { 1 } else { 0 }).into_par_iter().map(
            |i| {
                Self::cm_commit(
                    &ck.comm_key,
                    &batch_polynomial.coeffs[i * n..core::cmp::min((i + 1) * n, p)],
                    None,
                    None,
                ).into_affine()
            }
